...
has sender =>
    (is => 'rw', default => sub { PagSeguro::API::Payment::Sender->new });
...
sub request {
    ...
    my $res
        = $req->post(url => $self->api_uri . '/checkout/', params => $params);
...
sub session {
    ...
    my $req = PagSeguro::API::Request->new;
    my $res
        = $req->post(url => $self->api_uri . '/sessions/', params => $params);

    # parse response
    if ($res && !$res->error) {
        my $data   = $res->data;
        my $result = $self->_parse_sessions($data);
...
sub request_form {
    ...
    my $html
        = qq{<form method="post" target="pagseguro"  action="$base_uri/v2/checkout/payment.html">\n};
...
sub _build_params {
    ...
    my $c = 1;
    for my $i (@{$self->items}) {
        map { my $k = camelize($_) . $c; $params->{"item$k"} = $i->{$_} }
            keys %$i;
        $c++;
    }
...
sub _parse_request {
    ...
    return {
        code        => $code,
        date        => $date,
        payment_url => $self->base_uri
            . "/v2/checkout/payment.html?code=$code"
    };
}
...
sub _parse_sessions {
    ...
    # getting /session/id
    my $sessid = $xml->find('/session/id')->string_value;
...
PagSeguro::API::Payment - PagSeguro API payment class
...
PagSeguro API implementation for payment.
